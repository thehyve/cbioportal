...
public class DaoTreatment {
    ...
    public static Treatment addTreatment(Treatment treatment) throws DaoException {
        ...
        int geneticEntityId = DaoGeneticEntity.addNewGeneticEntity(DaoGeneticEntity.EntityTypes.TREATMENT);
        treatment.setGeneticEntityId(geneticEntityId);

        DbContainer container = executeSQLstatment(
            SqlAction.INSERT,
            "INSERT INTO treatment (`STABLE_ID`, `NAME`, `DESCRIPTION`, `LINKOUT_URL`, `GENETIC_ENTITY_ID`) "
            + "VALUES(?,?,?,?,?)",
            treatment.getStableId(),
            treatment.getName(),
            treatment.getDescription(),
            treatment.getRefLink(),
            String.valueOf(treatment.getGeneticEntityId())
        );
    ...
    private static void deleteTreatmentGeneticProfiles() throws DaoException {
        executeSQLstatment(
            SqlAction.DELETE,
            "DELETE FROM genetic_profile WHERE GENETIC_ALTERATION_TYPE = 'TREATMENT_RESPONSE'"
        );
    }
    
    /**
     * Deletes genetic_profile_link records which are pointing to a profile of type to TREATMENT_RESPONSE genetic_alteration_type
     * @throws DaoException 
     */
	private static void deleteTreatmentGeneticProfileLinks() throws DaoException {
		executeSQLstatment(
        	...
        	+ "(SELECT GENETIC_PROFILE_ID FROM genetic_profile WHERE GENETIC_ALTERATION_TYPE = 'TREATMENT_RESPONSE')");
	}    
