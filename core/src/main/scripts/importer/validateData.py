    ...
    def skipValidation(self, data):
        ...
        if hugo_symbol == 'Unknown' and entrez_id == '0':
            ...
            if variant_classification == 'IGR':
                self.logger.info("cBioPortal does not support variants with Variant Classification 'IGR'. "
                                 "This variant will be filtered out",
                                    extra={'line_number': self.line_number,
        ...
        if self.meta_dict.get('swissprot_identifier', 'name') == 'accession':
            if not re.match(
                    ...
                    r'[A-NR-Z][0-9]([A-Z][A-Z0-9]{2}[0-9]){1,2})$',
                     value):
                # Return this as a warning. The cBioPortal front-end currently (1.10.2) does not use the SWISSPROT
