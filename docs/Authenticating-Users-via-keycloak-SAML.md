...
the top while configuring the `cbioportal` client – _not_ the button
of the same name in the left sidebar.  Click the **Add Role**
button. Enter a name (e.g.  `brca_tcga_pub`) and description for the
role and hit the **Save** button.
...
after they have logged in for the first time) and click the ID, and
