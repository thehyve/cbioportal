...
public class MolecularDataServiceImpl implements MolecularDataService {
    ...
    public List<GeneMolecularData> getMolecularDataInMultipleMolecularProfilesByGeneQueries(List<String> molecularProfileIds,
        ...
        // molecularProfile->entrezGeneId->alterationType lookup table for CNA events
        Map<String, Map<Integer, Map<Integer, String>>> cnaEventLookup = copyNumberData.stream()
            .collect(groupingBy(d -> d.getMolecularProfileId(),
        ...
        molecularDataList = molecularDataList.stream().filter(
            d -> cnaEventLookup.containsKey(d.getMolecularProfileId())
                ...
                && cnaEventLookup.get(d.getMolecularProfileId()).get(d.getEntrezGeneId()).containsKey(Integer.parseInt(d.getValue()))
        ).collect(Collectors.toList());
