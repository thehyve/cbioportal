...
public class TreatmentDataServiceImplTest extends BaseServiceImplTest {
    ...
    private Float PIVOT_THRESHOLD = 0.1f;
    ...
    public void setUp() throws Exception {
        ...
        TreatmentMolecularAlteration treatmentGeneticAlteration = new TreatmentMolecularAlteration();
        treatmentGeneticAlteration.setTreatmentId(GENESET_ID1);
        treatmentGeneticAlteration.setValues("0.2,0.499");
        treatmentGeneticAlteration.setPivotThreshold(PIVOT_THRESHOLD);
        treatmentGeneticAlteration.setSortOrder(SORT_ORDER);
        
        treatmentGeneticAlterationList.add(treatmentGeneticAlteration);

        treatmentGeneticAlteration = new TreatmentMolecularAlteration();
        treatmentGeneticAlteration.setTreatmentId(GENESET_ID2);
        treatmentGeneticAlteration.setValues("0.89,-0.509");
        treatmentGeneticAlteration.setPivotThreshold(PIVOT_THRESHOLD);
        treatmentGeneticAlteration.setSortOrder(SORT_ORDER);
    ...
    public void fetchTreatmentData() throws Exception {
       ...
       TreatmentMolecularDataProfile profile = treatmentDataService.fetchTreatmentData(MOLECULAR_PROFILE_ID, Arrays.asList(SAMPLE_ID1, SAMPLE_ID2),
        ...
        Assert.assertEquals(PIVOT_THRESHOLD, profile.getPivotThreshold());
        assertEquals(SORT_ORDER, profile.getSortOrder());
        Assert.assertEquals( MOLECULAR_PROFILE_ID, profile.getMolecularProfileId() );

        List<TreatmentMolecularData> result = profile.getDataPoints();
        ...
        profile = treatmentDataService.fetchTreatmentData(MOLECULAR_PROFILE_ID, Arrays.asList(SAMPLE_ID1),
                Arrays.asList(GENESET_ID1, GENESET_ID2));
        Assert.assertEquals(PIVOT_THRESHOLD, profile.getPivotThreshold());
        assertEquals(SORT_ORDER, profile.getSortOrder());
        Assert.assertEquals( MOLECULAR_PROFILE_ID, profile.getMolecularProfileId() );
        
        result = profile.getDataPoints();
